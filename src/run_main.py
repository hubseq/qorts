...
def runOtherPre( input_dir, output_dir, run_json ):
    ...
    pargs_list_unsorted = pargs.split(' ')
    pargs_list = []
    # fix argument order - pargs come first
    for parg in pargs_list_unsorted[1:]:
        if parg[0:2]=='--':
            pargs_list = [parg] + pargs_list
        else:
            pargs_list.append(parg)
    # pargs_list = ['Rscript','/run_program.R'] + pargs_list[1:]
    pargs_list = ['java', '-jar', '/QoRTs.jar', 'QC'] + pargs_list
